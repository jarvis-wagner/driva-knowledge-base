# Geolocalizacao

## Descrição Geral
Geolocalizacao - Datapack da Driva com dados estruturados para inteligência de mercado.

## Dados e Campos Disponíveis
Total de campos: 78

### Campos Principais

| Campo | Descrição |
|-------|----------|
| ------- | ---------- |
| NOME ESTABELECIMENTO | Nome do estabelecimento físico. |
| CATEGORIAS ESTABELECIMENTO | Categorias do estabelecimento. |
| CATEGORIA PRINCIPAL | Categoria principal. |
| DESCRIÇÃO | Descrição do estabelecimento. |
| TELEFONE | Telefone formatado. |
| WEBSITE | Site do estabelecimento. |
| LINK CARDÁPIO | Link do cardápio (restaurantes). |
| FOTO | URL da foto principal. |
| STATUS NEGÓCIO | Status do negócio (aberto/fechado). |

*E mais 69 campos adicionais...*

## Planos Disponíveis
- ** Plano Inteligência

## Casos de Uso Práticos
Este datapack pode ser utilizado para:
- Enriquecimento de bases de dados existentes
- Análise de mercado e inteligência comercial
- Segmentação e targeting de clientes
- Due diligence e análise de risco

## Público-Alvo
- Empresas de tecnologia
- Consultorias e agências
- Instituições financeiras
- Departamentos de vendas e marketing

## Integração com Outros Produtos
Este produto pode ser combinado com outros datapacks da Driva para análises mais abrangentes e insights mais profundos.

## Informações Técnicas
- Formato: API REST e exportação em formatos estruturados
- Atualização: Conforme especificação do produto
- Volume: 78 campos disponíveis

## Última Atualização
Processado em 2026-02-04 14:14:54 UTC
